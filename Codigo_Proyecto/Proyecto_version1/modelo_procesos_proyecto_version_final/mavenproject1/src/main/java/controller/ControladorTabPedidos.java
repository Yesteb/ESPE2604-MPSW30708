...
class ControladorTabPedidos {
    ...
    void construir() {
        ...
        btnAgregar.setBackground(EstiloUI.COLOR_PRIMARIO);
        btnAgregar.setForeground(EstiloUI.COLOR_MUY_OSCURO);
        ...
        panel.add(crearBarraRefrescar(), BorderLayout.NORTH);
        panel.add(desplazable, BorderLayout.CENTER);
        panel.revalidate();
    }

    /**
     * Barra superior con el botón de refrescar. Recarga las etiquetas y las
     * categorías personalizadas desde la base de datos <em>sin</em> reconstruir
     * el formulario: lo que el usuario tenga escrito (cliente, precio,
     * descripción) se conserva.
     */
    private JPanel crearBarraRefrescar() {
        JButton btnRefrescar = EstiloUI.crearBotonAccion("Refrescar",
            EstiloUI.COLOR_NEUTRO_CLARO, EstiloUI.COLOR_MUY_OSCURO, EstiloUI.COLOR_NEUTRO);
        btnRefrescar.setToolTipText("Vuelve a leer las etiquetas de la base de datos");
        btnRefrescar.addActionListener(e -> refrescar());

        JPanel barra = new JPanel(new FlowLayout(FlowLayout.RIGHT, 4, 6));
        barra.setBorder(new EmptyBorder(4, 8, 0, 8));
        barra.add(btnRefrescar);
        return barra;
    }

    /** Vuelve a leer de la base de datos las etiquetas y categorías del formulario. */
    void refrescar() {
        cargarCategoriasCustom();
        recargarChips();
    }
    ...
    void recargarChips() {
        ...
        // ESTILO y TALLA ya no tienen sección propia en el formulario: se
        // gestionan como categorías personalizadas. Sus paneles no llegan a
        // crearse, así que solo se reconstruyen si existen. Sin esta guarda,
        // recargarChips() lanzaba NullPointerException y dejaba los chips sin
        // actualizar cada vez que se añadía o borraba una etiqueta.
        if (panelChipsEstilo != null)
            construirChipsEstilo(repoEtiqueta.buscarPorCategoria("ESTILO"));
        if (panelChipsTalla != null)
            construirChipsTalla(repoEtiqueta.buscarPorCategoria("TALLA"));
    ...
    private JPanel construirFormulario() {
        ...
        buscadorCliente = new BuscadorCliente(servicioPedido::buscarClientes);
        panelCliente.add(buscadorCliente, BorderLayout.CENTER);
        ...
        ((javax.swing.text.AbstractDocument) txtCantidad.getDocument()).setDocumentFilter(new javax.swing.text.DocumentFilter()
        {
            @Override
    ...
    private void onAgregarProducto() {
        String cliente   = buscadorCliente.getTexto();
        ...
        if (cliente.isEmpty()) {
            ...
            buscadorCliente.getTextField().requestFocus();
            return;
        }
        if (tipoSel == null) {
